...
def randomcut():
    ...
    for name in namelist:
        data = processjson('my_%s' %name)
        newdata = random.sample(data,int(1/13*len(data)))
        with open('./textfile/'+name+'cut.json', 'w') as f:
            for item in newdata:
                json.dump(item, f)
            print('new %s set has %d samples' %(name,len(newdata)))

def generate_finegrainedsamples():
    '''
    train:datasetname_coarse_train.txt
    test:datasetname_coarse.txt
    '''
    type=['ami_train','ami_test']
    for name in type:
        infolist = pickle.load(open('%s_anno.pkl'%name,'rb' ))
        filename = name+'_annobase'
        text = merge_predictions('%s.txt' %filename,infolist)
        data=processjson(name)
        num = len(text)
        print(num)
        jsonfile = open('./textfile/%s_f_annobase.json'%name,'w' )
        for index in range(num):
            tmp=dict()
            # tmp['publication_number']=0
            # tmp['application_number']=0
            tmp['description']=text[index]
            tmp['abstract']=data[index]['abstract']
            json.dump(tmp,jsonfile)
    

def merge_predictions(filename,infolist):
    file = open(filename,'r')
    textlist = file.readlines()
    result = list()
    p = 0
    for length in infolist:
        tmp =list2txt(textlist,p,p+length)
        result.append(tmp)
        p=p+length
    return result
...
def main():
    ...
    for name in namelist:
        file = open('ami/%s.json' %name)
        data = file.read()
        data = json.loads(data)
        newfile = open('textfile/ami_%s.json' %name,'w')
        for item in data:
            tmp = dict()
            tmp['description'] = item[1]
            tmp['abstract'] = item[0].replace('<t>','').replace('</t>','').replace('    ',' ')
            json.dump(tmp,newfile)

if __name__=='__main__':
    generate_finegrainedsamples()